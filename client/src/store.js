...
export const useStore = create((set) => ({
  ...
  getUserLastPurchase: async (userId) => {
    // No se si es necesario ya q el carrito ya persiste a menos q sea de otra cosa
    try {
  ...
  getFilteredReviews: async () => {
    try {
      const { data } = await axios.post(
        `${VITE_BACK_URL}/resenas`,
        useStore.getState().filtrosResenas
      );
      set((state) => ({
        ...state,
  ...
  getReviewedProducts: async () => {
    try {
      const { data } = await axios.post(
        `${VITE_BACK_URL}/productoResena`,
        useStore.getState().userInfo.reviews
      );
      set((state) => ({
        ...state,
        userInfo: {
